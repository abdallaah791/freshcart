...
export default function RecentProducts() {
  ...
  function handleSearch(event) {
    ...
    setSearchTerm(term);
    setFilteredProducts(products.filter(product => product.title.toLowerCase().includes(term)));
  }
              ...
              <div key={product.id} className="w-1/6">
                <div className="products p-2 rounded-lg relative group p-4 bg-white transition-all cursor-pointer hover:text-white hover:shadow-lg hover:scale-105">
                  <Link to={`/productdetails/${product.id}`}>
                    <img src={product.imageCover} className="w-full object-top object-cover" alt={product.title} />
                  ...
                  <button
                    ...
                    className={`w-full p-4 border rounded-lg shadow-sm transition-all cursor-pointer ${loadingProduct === product.id ? 'bg-gray-400 cursor-not-allowed' : 'bg-white hover:bg-green-500 hover:text-white hover:shadow-lg'} opacity-0 group-hover:opacity-100 transform group-hover:translate-y-0 translate-y-2 duration-300`}
                  >
